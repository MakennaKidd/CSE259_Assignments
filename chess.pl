% Name: Makenna Kidd
% Project: Project 2 - Chess Game
% Prof: Osburn
% Class: CSE259
% Due Date: April 4, 2021 @ 11:59pm
...
% Per recitation, this code was to be modified from manual play to automation play
play(Board) :-
...
% Used to catch unexpected situations
...
% Predicate that allows board to be printed
print_board(InBoard) :-
draw_board(InBoard,8,8).

% Recursion predicate that draws symbols that are required to draw chess board
drawSymbol(Symbol, 0).
drawSymbol(Symbol, N) :-
    N > 0,
    write(Symbol),
...
% Line 439 - 467 Predicate that draws border  '+', '-'
...
% Predicate for drawing pairs that are empty slots correlated with the letters							
...
% Pairing the letters and number rows and columns with the chess pieces
pair(a,8).
pair(b,7).
pair(c,6).
pair(d,5).
...
% Checking member predicates	
mymember(X, [X|_]).
mymember(X, [_|L]) :- mymember(X, L).
	
% Function for stating the colors, pieces, letter and number combination	
init_board( [
 	state(white, WhiteKing, WhiteKingRook, WhiteQueenRook),
	state(black, BlackKing, BlackKingRook, BlackQueenRook),
